...
pnpm desktop:start                      # 构建后用 Electron 打开（验证 app:// 打包形态）
pnpm desktop:dev                        # Electron 连 dev server 调 UI（需先另开 pnpm dev）
pnpm desktop:build                      # 打 macOS dmg → release/
```
...
**当前 db 版本：v9**（src/db/index.ts）。**Schema 演进**：每次表结构变更都通过 `this.version(N).stores({...}).upgrade(async tx => ...)` 增加新版本号，迁移函数集中在 `src/db/migrations/`。**绝对不要原地修改旧版本号的 stores**，会破坏老用户数据。
...
- `searcher.ts` — 顶部搜索的「自然语言意图 → 命中资产」AI 匹配（aiMode 路径）
- `keywordMatch.ts` — 关键字模式下的字段权重打分 + 排序 + 命中片段切分。导出 `scorePrompt / scoreTaskPack / scoreContext`（权重 title=4 / summary|goal=2 / tags=2 / body|description|content=1）、`topKeywordHits` 包装排序与截断、`snippetAroundMatch` 提供下拉条目里的命中片段预览。**全仓所有关键字搜索入口都应该走这里**（顶部 SearchResultsDropdown、usePrompts 列表、WorkbenchSidebar、AddPromptPopover、/contexts 页等），不要再各处自己写 `title.includes() || body.includes() || ...` 否则字段策略变更会再次散落不一致
...
- `backup.ts` — 全量备份与跨设备同步（Settings 页）。底层 `collectBackup()` 读五表组装 `BackupFile`，被下列入口共用：`exportAllData`（→ 下载 JSON）、`saveSnapshotToRepo`（写 `public/data-snapshot.json` **并自动 git add/commit/push**。两条宿主路径：web 端 **仅 dev**，POST `/__save-snapshot` 走 vite.config.ts 的 `snapshotWriter` 中间件，生产构建无此端点会 404；桌面端走 Electron IPC `snapshot:save`，写用户在设置页绑定的仓库目录。写盘+git 的实现只有一份，在 `scripts/snapshotRepo.mjs`，两个宿主共用）、`importBackupFromFile`（**合并**导入，id 冲突自动重命名而非覆盖）、`restoreBackupFromFile`/`restoreFromRepoSnapshot`（**覆盖式**恢复，共用 `applyRestore`：清空五表 + bulkPut，清空前必经 `writeMigrationArchive` 归档可回滚）。**两条数据流必须分清**：① 自用·多设备同步走 `public/data-snapshot.json` 覆盖式镜像（last-writer-wins）；② 商业·分发给客户走 seed 三件套（幂等 probe，零覆盖），**绝不能拿 data-snapshot.json 当分发通道**——客户点「从仓库恢复」会被 `applyRestore` 清空自己编辑的数据。**自用同步 SOP**：A 台（dev）点「保存快照到仓库」→ 中间件自动 `git add/commit/push`（commit 用 pathspec 只提交快照文件，文件无变化则跳过不产生空提交；push 失败只回传 detail 不阻塞，UI 提示并保留手动命令兜底）；B 台 `git pull` 后点「从仓库恢复最新数据」一键镜像（无需选文件）。**中间件落盘坑**：接收 request body 必须 `Buffer.concat(chunks)` 后再 `toString("utf8")`，绝不能 `body += chunk`——快照含大量中文，多字节字符跨 chunk 边界会被逐块解码损坏（已踩，见 vite.config.ts 注释）
- `desktop.ts` — Electron 桌面壳能力桥。导出 `desktop`（web 端为 `null`）与 `canWriteRepo`（能否直接写仓库快照 = `import.meta.env.DEV || 桌面端`）。**判断是否运行在桌面端全仓只走这里**，不要各处摸 `window.promptOSDesktop` 或 userAgent
...
8. **搜索命中正文后的"定位 + 高亮"用 DOM Range.surroundContents，不走 rehype plugin** — `PromptDetail.tsx` 在 body 容器上挂 `useEffect`，监听 `selectedPromptId / searchQuery / p?.body`，用 `TreeWalker(SHOW_TEXT)` 找第一处命中、`Range.surroundContents(mark)` 包成 `<mark.search-highlight>` 再 `scrollIntoView`；effect cleanup 把 mark 拆掉还原文本节点。**为什么不在 MarkdownView 里加 rehype 高亮插件**：rehype 会让 ReactMarkdown 重渲染时 mark 节点经过 React reconciler，跨组件重渲染（PromptDetail 多个 local state）容易把 mark 擦掉；DOM 层直接操作 + ref 跟踪反而稳定。同样的"轻量"片段高亮在 `SearchResultsDropdown` 下拉里走的是另一条路径：`snippetAroundMatch` 切出 `{before, match, after}` 三段，React 渲染 `<mark.dropdown-highlight>` —— 这条路径是声明式的，没有 surroundContents 那套。两套 mark CSS 在 `src/index.css`（`.search-highlight` 带 amber 闪烁动画，`.dropdown-highlight` 静态轻量），不要混用
9. **桌面端用 `app://` 自定义协议 + SPA 回退，不是 `file://` + HashRouter** — `electron/main.mjs` 里 `registerSchemesAsPrivileged({standard:true, secure:true})` 注册 `app`，再用 `protocol.handle` 从 `dist/` 提供文件、找不到且无扩展名时回退 `index.html`。**换来三件事**：① origin 稳定且是安全上下文 → IndexedDB/localStorage 能持久化、跨版本仍是同一个库；② `createBrowserRouter` 与 `fetch("/seed-prompts.json")`、`${BASE_URL}data-snapshot.json` 这类站点根绝对路径**零改动**（不用改 vite `base`、不用换 HashRouter）；③ 路由页刷新不白屏。**代价**：dist 必须与 electron/ 一起进包（见 electron-builder.yml 的 files）。**注意** web 端与桌面端 origin 不同 → 数据天然隔离，两端同步只能走仓库快照，不存在"自动共享"
10. **桌面端仓库目录靠用户显式绑定，不猜路径** — 打包后的 app 不知道仓库在哪，路径存在 `userData/desktop-config.json`，由设置页「选择仓库目录…」经原生 dialog 写入（校验含 `.git` 与 `public/`）。未绑定时：保存快照报可执行的错误文案；`peekRepoSnapshot`/`restoreFromRepoSnapshot` 回退到**随包分发的 dist 副本**——这条回退正好承担首次数据迁移。绑定后读的是仓库目录里的**实时文件**，`git pull` 后立刻生效
